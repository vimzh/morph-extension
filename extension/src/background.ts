...
const SIDEPANEL_STATE_KEY_PREFIX = 'sidepanelState:'
...
async function readSidepanelState(tabId: number) {
  if (sidepanelStateByTab.has(tabId)) {
    return { known: true, isOpen: Boolean(sidepanelStateByTab.get(tabId)) }
  }
  const key = `${SIDEPANEL_STATE_KEY_PREFIX}${tabId}`
  const stored = await chrome.storage.session.get(key)
  if (!(key in stored)) return { known: false, isOpen: false }
  const isOpen = Boolean(stored[key])
  sidepanelStateByTab.set(tabId, isOpen)
  return { known: true, isOpen }
}

async function writeSidepanelState(tabId: number, open: boolean) {
  sidepanelStateByTab.set(tabId, open)
  await chrome.storage.session.set({ [`${SIDEPANEL_STATE_KEY_PREFIX}${tabId}`]: open })
}

async function deleteSidepanelState(tabId: number) {
  sidepanelStateByTab.delete(tabId)
  await chrome.storage.session.remove(`${SIDEPANEL_STATE_KEY_PREFIX}${tabId}`)
}

async function ensureContentScript(tabId: number) {
  try {
    await sendSidepanelState(tabId, true)
  } catch {
    const script = chrome.runtime.getManifest().content_scripts?.[0]?.js?.[0]
    if (!script) return
    await chrome.scripting.executeScript({ target: { tabId }, files: [script] }).catch(() => {})
  }
}

async function setSidepanelState(tabId: number, open: boolean) {
  await writeSidepanelState(tabId, open)
  if (open) {
    await ensureContentScript(tabId)
    return
  }
  await sendSidepanelState(tabId, false).catch(() => {})
}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (type === MESSAGE_TYPES.getSidepanelState) {
    if (tabId) {
      readSidepanelState(tabId).then(sendResponse)
      return true
    }
    sendResponse({ isOpen: false, known: false })
    return true
  }
...
chrome.tabs.onRemoved.addListener((tabId) => {
  consoleLogsByTab.delete(tabId)
  void deleteSidepanelState(tabId)
})
...
chrome.tabs.onActivated.addListener(async ({ tabId }) => {
  const { isOpen } = await readSidepanelState(tabId)
  await sendSidepanelState(tabId, isOpen).catch(() => {})
})

chrome.tabs.onCreated.addListener(async (tab) => {
  if (typeof tab.id !== 'number') return
  await sendSidepanelState(tab.id, false).catch(() => {})
})
