...
const STYLE_ID = 'extension-hover-highlight-style'
const OVERLAY_ID = 'extension-hover-highlight-overlay'
const LABEL_ID = 'extension-hover-highlight-label'
const CLICK_OVERLAY_ID = 'extension-click-highlight-overlay'
const CLICK_LABEL_ID = 'extension-click-highlight-label'
const CLICKED_CLASS = 'extension-clicked-highlight'
...
function isExtensionUi(el: Element): boolean {
  return Boolean(el.closest('#extension-app'))
}
...
async function clearAllClickedHighlights() {
  ...
  await chrome.runtime.sendMessage({
    type: MESSAGE_TYPES.clearClicked,
    url: window.location.href,
  })
}
...
async function syncSidepanelState() {
  try {
    ...
    // A later side-panel lifecycle message can still enable the highlighter.
  }
}
